...
class PlaybookRunner:
    ...
    async def run_for_case(self, case_id: str):
        ...
        for step in steps:
            ...
            if fresh and fresh["status"] in ("resolved", "closed", "escalated"):
                return
    ...
    async def _execute_step(self, step, case_id, case, shipment, exceptions):
        ...
        if delay and delay > 0:
            self._log_event(case_id, shipment["id"], "step.waiting",
                "ai", f"Waiting before: {action.replace('_', ' ')}")
            await asyncio.sleep(min(delay * 60, 10))

        if owner == "human":
            self._create_human_task(case_id, config.get("title", action), config.get("reason", ""))
            self._log_event(case_id, shipment["id"], "task.created",
                "ai", f"Task created for human: {config.get('title', action)}")
            if config.get("reason") or action == "escalate":
                self._maybe_escalate(case_id, shipment["id"], config.get("reason", "Human action required"))
            return
        ...
        if action == "send_email":
            ...
            self._create_ai_task(case_id, config.get("title", "Follow up"))
            self._log_event(case_id, shipment["id"], "task.created",
                "ai", f"AI task created: {config.get('title', 'Follow up')}")
        elif action == "escalate":
            self._maybe_escalate(case_id, shipment["id"], config.get("reason", "Escalation threshold reached"))

    def _maybe_escalate(self, case_id, shipment_id, reason):
        self._update_case_status(case_id, "pending_human")
        self._log_event(case_id, shipment_id, "case.escalated", "ai", f"Escalated: {reason}")

    def _create_human_task(self, case_id, title, description=""):
        self.db.table("tasks").insert({"case_id": case_id, "owner": "human",
            "title": title, "description": description, "status": "pending"}).execute()

    def _create_ai_task(self, case_id, title):
        self.db.table("tasks").insert({"case_id": case_id, "owner": "ai",
            "title": title, "status": "pending"}).execute()
    ...
    def _log_event(self, case_id, shipment_id, event_type, actor, summary, payload={}):
        self.db.table("events").insert({"case_id": case_id, "shipment_id": shipment_id,
            "event_type": event_type, "actor": actor, "summary": summary, "payload": payload}).execute()
    ...
    def _get_playbook(self, exception_type):
        r = self.db.table("playbooks").select("*").eq("exception_type", exception_type).eq("enabled", True).limit(1).execute()
